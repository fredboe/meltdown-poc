...
FlushReload fr_init(void) {
    ...
    for (size_t i = 0; i < N_BYTE_VALUES * PAGE_SIZE; i++) buffer[i] = 219;
...
void fr_reset(const FlushReload *fr) {
    for (size_t page_start = 0; page_start < N_BYTE_VALUES * PAGE_SIZE; page_start += PAGE_SIZE) {
...
uint8_t fr_get(const FlushReload *fr) {
    ...
    for (size_t i = 0; i < N_BYTE_VALUES; i++) {
